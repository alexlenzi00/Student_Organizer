...
public class SQLiteLoader implements Loader {
	...
	public <T extends Savable> ArrayList<T> loadAll(String tableName) {
		ArrayList<T> ris = new ArrayList<>();
		try {
			Class<T> c = (Class<T>) Class.forName("model." + tableName);
	...
	public <T extends Savable> T load(String tableName, int id) {
